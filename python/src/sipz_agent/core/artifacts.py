...
from sipz_agent.schemas.citations import CandidateCitation, RejectedCitation, RetrievalQueryPlan
...
class StudyArtifacts:
    def __init__(
        ...
        *,
        ...
        rejected_sources: list[RejectedCitation] | None = None,
...
def write_rejected_sources_markdown(path: Path, rejected_sources: list[RejectedCitation]) -> None:
    lines = ["# Rejected Candidate Sources", ""]
    if not rejected_sources:
        lines.extend(["No candidate sources were rejected.", ""])
    for index, rejected in enumerate(rejected_sources, start=1):
        source = rejected.citation
        ...
        lines.append(f"- Source: {source.source}")
        lines.append(f"- Retrieval query: {source.retrieval_query}")
        lines.append(f"- Rejection code: {rejected.rejection_code}")
        lines.append(f"- Screening confidence: {rejected.screening_confidence:.2f}")
        if rejected.matched_alias:
            lines.append(f"- Matched alias: {rejected.matched_alias}")
        if rejected.failed_requirement:
            lines.append(f"- Failed requirement: {rejected.failed_requirement}")
        lines.append(f"- Human health relevance: {screening.human_health_relevance}")
        lines.append(f"- Mentions nutrient/bioactive: {screening.mentions_nutrient_or_bioactive}")
        lines.append(f"- Conclusiveness: {screening.conclusiveness}")
        lines.append(f"- Rationale: {screening.rationale}")
        lines.append("")
    path.write_text("\n".join(lines), encoding="utf-8")
...
def write_study_artifacts(out_dir: str | Path, run_id: str, artifacts: StudyArtifacts) -> Path:
    ...
    if artifacts.query_plan is not None:
        write_json(run_dir / "query_plan.json", artifacts.query_plan.model_dump(mode="json"))
    ...
    write_json(run_dir / "rejected_sources.json", model_dump_jsonable(artifacts.rejected_sources))
    write_rejected_sources_markdown(run_dir / "rejected_sources.md", artifacts.rejected_sources)
